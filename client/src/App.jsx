...
function App() {
  ...
  const checkVaultAuth = async () => {
    try {
      ...
      console.error('Failed to verify vault session:', err);
      setIsVaultAuthenticated(false);
    } finally {
      setIsChecking(false);
    }
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-xs font-label-mono text-text-muted uppercase tracking-widest">
            INITIALIZING VAULT...
          </p>
        </div>
      </div>
  ...
  if (!isVaultAuthenticated) {
    return <VaultLogin onLoginSuccess={() => setIsVaultAuthenticated(true)} />;
  }
